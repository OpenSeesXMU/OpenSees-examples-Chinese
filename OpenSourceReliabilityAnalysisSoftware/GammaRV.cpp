...
GammaRV::getInverseCDFvalue(double probValue)
{
	...
	for (int i=1;  i<=100;  i++ )  {
		...
		if (fabs(1.0-fabs(x_old/x_new)) < tol) {
			...
			if (i==100) {
				cout << "WARNING: Did not converge to find inverse CDF!" << endl;
				result = 0.0;
			}
			else {
			x_old = x_new;
...
double GammaRV::getParameter3()  {cout<<"No such parameter in r.v. #"<<tag<<endl; return 0.0;}
double GammaRV::getParameter4()  {cout<<"No such parameter in r.v. #"<<tag<<endl; return 0.0;}
...
GammaRV::gammaFunction(double x)
{
	...
	if (x==0 || ( x < 0.0 && floor(x)==x ) )  {
		cout << "Invalid input to the gamma function" << endl;
	}
	else {
		Vector p(9);
		Vector q(9);
