...
GFunVisualizationAnalysis::analyze(void)
{
	...
	cout << "Visualization Analysis is running ... " << endl;;
		...
		for (j=1; j<=numPoints2; j++) {
			...
			if (funSurf==1) {
				...
				if (space==2) {
					...
					if (result < 0) {
						cout << "GFunVisualizationAnalysis::analyze() - " << endl
	...
	cout << endl << "GFunVisualizationAnalysis completed." << endl;
...
GFunVisualizationAnalysis::findGSurface(Vector thePoint)
{
	...
	if (dir==1) {
		...
		if (nrv != theDirectionVector.Size()) {
			cout << "ERROR: There is something wrong with the size of the direction" << endl
				<< " vector of the visualization analysis object." << endl;
		}
