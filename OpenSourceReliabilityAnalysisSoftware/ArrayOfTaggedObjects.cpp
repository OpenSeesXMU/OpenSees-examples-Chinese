 ...
 myIter(*this)
{
    ...
    if (theComponents == 0) {
      cout << "ArrayOfTaggedObjects::ArrayOfTaggedObjects - failed to allocate an array of size " << size << endl;
    } else 
...
ArrayOfTaggedObjects::setSize(int newSize)
{
    ...
    if (newSize < 0 && newSize > sizeComponentArray) {
      cout << "ArrayOfTaggedObjects::setSize - invalid size " << newSize << endl;
      return -1;	
    } 
	...
	// cannot increase size to newSize, keep as is and return error
      cout << "ArrayOfTaggedObjects::setSize - failed to allocate an array of size " << newSize << endl;
      return -2;
    } 
	...
	for (int j=0; j<=oldArrayLastEntry; j++)
		...
		if (this->addComponent(oldArray[j]) == false) {
		  ...
		  cout << "SERIOUS ERROR: ArrayOfTaggedObjects::setSize() - we have lost a component with tag: " <<
		    oldArray[j]->getTag() << endl;
		  error = -3;
		}    
    }
...
//    if (allowMultiple == false) {
	...
	if (other != 0) {
	  cout << "WARNING ArrayOfTaggedObjects::addComponent() - component" <<
	    " with tag already exists, not adding component with tag: " <<
	    newComponent->getTag() << endl;
	  return false;
	}
//    }
	...
	if (this->setSize(2*numComponents) < 0) {
	  cout << "ArrayOfTaggedObjects::addComponent()- failed to enlarge the array with size" <<
	    2*numComponents << endl;
	  return false;
	}
    ...
    if (positionLastNoFitEntry == sizeComponentArray) {
      cout << "ArrayOfTaggedObjects::addComponent() - could not - find a vacant spot after enlarging!!\n";
      return false;
    }
...
ArrayOfTaggedObjects::getEmptyCopy(void)
{
    ...
    if (theCopy == 0) {
      cout << "ArrayOfTaggedObjects::getEmptyCopy - out of memory\n";
    }
...
// Quan -- change this is DANGEOUS.. remember to recover later.

int ArrayOfTaggedObjects::setNumComponents(int pNum)
{
	numComponents = pNum;
	return 0;
}
