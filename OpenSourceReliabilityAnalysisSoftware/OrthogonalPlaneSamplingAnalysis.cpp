...
OrthogonalPlaneSamplingAnalysis::OrthogonalPlaneSamplingAnalysis(   Tcl_Interp *interp,
	...
	if (zeroFindingType ==1) { //safeGuardedZeroFinding..
		...
		cout<<"unknown zerofinding algorithm type.."<<endl;
		exit(-1);
	}

	if ((analysisTypeTag ==1)||(analysisTypeTag ==2)){
		double * tmp = new double[100]; // this may be unnecessary
		double * tmp1 = new double[100];// this may be unnecessary
		theAlgorithm->setX1Pointer(tmp);// this may be unnecessary
		theAlgorithm->setG1Pointer(tmp1);// this may be unnecessary
		theAlgorithm->set_ii_1(0);
	}
	if (analysisTypeTag ==2){
		double * tmp2 = new double[100]; // this may be unnecessary
		double * tmp3 = new double[100];// this may be unnecessary
		theAlgorithm->setX2Pointer(tmp2);// this may be unnecessary
		theAlgorithm->setG2Pointer(tmp3);// this may be unnecessary
...
{
	cout << "OrthogonalPlaneSamplingAnalysis is running ... " << endl;


	

	ofstream resultsOutputFile( fileName, ios::out | ios::app );
	if ((printFlag ==1)||(printFlag ==2)) resultsOutputFile<< "Failure Probability" <<","<<"c.o.v."<<endl; 
...
//	}
	...
	if (theDesignPoint ==0) {cout<<"designpoint does not exist!"<<endl; exit(-1);}
	// note designPoint is in physical space ..

	result = theProbabilityTransformation->set_x(*theDesignPoint);
	if (result < 0) {
		cout << "OrthogonalSamplingAnalysis::analyze() - could not " << endl
	...
	//cout<<"theDesignPoint in x space is:"<<*theDesignPoint<<endl;
	//cout<<"theDesignPoint in u space is:"<<*theDesignPointInUSpace<<endl;
...
//	bool contribution;
	...
	if (numOfSimulations<2) {
		cout<<"warning: too less number of simulations, can not do simulation, adjust simulation number to 2.."<<endl;
		numOfSimulations =2;
	}
	...
	while( (k<=numOfSimulations && cov>targetCOV || k<=2) ) {
		...
		if (result < 0) {
			cout << "OrthogonalSamplingAnalysis::analyze() - could not generate" << endl
		...
		//cout.setPrecision(16);
		//cout<<"original sampling point u is:"<<u<<endl;		
	...
	// ---- step 2,compute U'_(n-1) by project U_n to the plane: alpha'*U_n =0 ---
		...
		//cout<<"uProjection is:"<<projectionPoint<<endl;
	...
	//	cout<<"the "<<k<<"th iteration. x is:" <<x<<endl;
...
//					theAlgorithm->setFEconvergence(true);
				...
				if (beta > uPrimeNorm){
					...
					if(result<0) {  // diverge
						cout<<"something wrong with orthogonalPlaneSamplingAnalysis. need save point "<<endl;
						needCallingZeroFinding =false; 
						//gFunctionValue = -1.0;
						contribution = false;
					// save point ....
						resultsOutputFile1<< "a:" <<a<<","<<"F(a): diverge"<<". uPrime:"<<uPrime<<endl;
						...
						if (fabs(Fa)<funcTol){
							...
							contribution = false;
							// save point ....
							resultsOutputFile1<< "a:" <<a<<","<<"F(a)"<<Fa<<". uPrime:"<<uPrime<<endl;
						...
						if (fabs(zeroPoint-b)<=varTol){  //possible diverge case
							...
							if(result<0) {  // diverge. evaluate value of current point 
								...
								if (gFunctionValue > funcTol){ 
									contribution = false;
									cout << "divergence caused by soil, but system safe. gFunctionValue is:" <<gFunctionValue<<endl;
...
//						if ( G2>=0) {contribution = false;} ?????  Quan Dec 23 2007
 		    ...
 		    if (contribution && needCallingZeroFinding){
					...
					if (theAlgorithm->findZeroPoint(x0)>=0){
						...
						printf("diverge!\n");
						resultsOutputFile1<<"zero1 diverge!. uPrime:"<<uPrime<<endl;
						contribution = false;
						// calling other like bisection method to make sure it converge!!
					}
...
// ================================finding the second zero point ==========================================
			if (contribution){
				...
				if (needCallingZeroFinding){
				   ...
				   if ( b == 999) { 
						...
						resultsOutputFile1<< "something wrong, G2: b does not exit.. uPrime:"<<uPrime<<endl;
						contribution = false;
				   }
				   ...
				   if (contribution && needCallingZeroFinding){
						...
						if (theAlgorithm->findZeroPoint(x0)>=0){
							...
							printf("========= diverge!\n something is wrong=========");
							resultsOutputFile1<< "something wrong, G2: can not find zero.. uPrime:"<<uPrime<<endl;
							contribution = false;
							// calling other like bisection method to make sure it converge!!
						}
			...
			if (contribution){
				...
				qk = (qk2-qk); 
				if (qk<0) {	resultsOutputFile1<< "something wrong, qk<0.. uPrime:"<<uPrime<<endl; qk=0.0;}
			}
...
// ====================== post processing ====================
	...
	ofstream resultsOutputFile_aa( "results__.out", ios::out);
	resultsOutputFile_aa<<probability<<"   "<<cov<<"   "<<numOfGFunEvaluations<<endl;
	...
	if ((printFlag ==1)||(printFlag ==2)){
		...
		if (printFlag ==2){
			...
			resultsOutputFile5<< probability <<endl;
			resultsOutputFile5<< cov         <<endl; 
			resultsOutputFile5<< k-1         <<endl;
			resultsOutputFile5<< seed        <<endl;
			resultsOutputFile5<< numOfGFunEvaluations <<endl;
	...
	cout << "numOfGFunEvaluations = " <<numOfGFunEvaluations<<"     numOfSimulations = "<<numOfSimulations<< endl;
			...
			/*
				...
				if (theLimitStateFunction == 0) {
					cout << "ImportanceSamplingAnalysis::analyze() - could not find" << endl
						<< " limit-state function with tag # 1." << endl;
					return -1;
				}
...
//			    printf( "expressionG1: %s\n", expressionG1 );
				...
				if (Tcl_Eval( theInterp, expression) !=TCL_OK ){cout<<"wrong.."<<endl; exit(-1);};
				...
				if (fabs(ud) <1.e-14) {
					cout<<" ----------------------------------------------------------------------------------" <<endl;
					cout<<"warning:ImportanceSamplingAnalysis::analyze velocity is 0! \n probabily because you are running static case!"<<endl;
					cout<<"-----------------------------------------------------------------------------------"<<endl;
				}
	...
	//			cout.precision(16);
	//			cout<<"G1:"<<gFunctionValue<<".  G2: "<<g2FunctionValue<<endl;
