...
/////////////////////////////////////////////////////////
	...
	if (Fa < -functionTol) {
		cout<<"error in SafeGuardedZeroFindingAlgorithm::findZeroPoint. Fa<0"<<endl; 
		return -1;
	}
...
//		x_new = a+0.15;
	...
	if (FEconvergence) {
		...
		if (fabs(Fx_new)<functionTol) {
			zeroPoint = x_new; 
			//cout<<"Return: x_new is:"<<x_new<<" Fx_new is: "<<Fx_new<<endl;
			return 1;
		}
		...
		if (Fx_new * Fa >functionTol*functionTol){ //good
			...
			cout<<"wrong bound!"<<endl; return -1;
		}
	}
	else {//diverge
		b=x_new; Fb=-1.0;
	...
	if (maxIterNum>100) {
		cout<<"warning: SafeGuardedZeroFindingAlgorithm::findZeroPoint may couse member ";
		cout<<" error since maxIterNum:"<<maxIterNum<<" exceed the limit 100..."<<endl;
	}
	...
	while (theSamplingAnalysis->getContribution() && /*fabs(Fx_new)>functionTol &&*/ iterationNum < maxIterNum && (b-a)>variableTol) {
		...
		if (FEconvergence){
			...
			else { cout<<"bound wrong"<<endl; return -1; }//cout<<"wrong bound!"<<endl; return -1;}
