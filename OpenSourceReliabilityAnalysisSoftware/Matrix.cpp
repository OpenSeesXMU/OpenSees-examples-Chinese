...
:numRows(0), numCols(0), dataSize(0), data(0), fromFree(0)
{
  ...
  if (matrixWork == 0) {
    ...
    if (matrixWork == 0 || intWork == 0) {
      cout << "WARNING: Matrix::Matrix() - out of memory creating work area's\n";
      exit(-1);
    }
  }
}
...
:numRows(nRows), numCols(nCols), dataSize(0), data(0), fromFree(0)
{
  ...
  if (matrixWork == 0) {
    ...
    if (dataSize > 0) {
      ...
      if (data == 0) {
	cout << "WARNING:Matrix::Matrix(int,int): Ran out of memory on init ";
	cout << "of size " << dataSize << endl;
...
:numRows(0), numCols(0), dataSize(0), data(0), fromFree(0)
{
  ...
  if (matrixWork == 0) {
    ...
    if (dataSize != 0) {
      ...
      // data = (double *)malloc(dataSize*sizeof(double));
      if (data == 0) {
	cout << "WARNING:Matrix::Matrix(Matrix &): ";
	cout << "Ran out of memory on init of size " << dataSize << endl; 
	numRows = 0; numCols =0; dataSize = 0;
      } else {
	// copy the data
	double *dataPtr = data;
...
Matrix::resize(int rows, int cols) {
  ...
  if (newSize <= 0) {
    cout << "Matrix::resize) - rows " << rows << " or cols " << cols << " specified <= 0\n";
    return -1;
  }
    ...
    // data = (double *)malloc(dataSize*sizeof(double));
    if (data == 0) {
      cout << "Matrix::resize(" << rows << "," << cols << ") - out of memory\n";
      numRows = 0; numCols =0; dataSize = 0;
      return -2;
    }
    dataSize = newSize;
...
Matrix::Assemble(const Matrix &V, const ID &rows, const ID &cols, double fact) 
{
  ...
  for (int i=0; i<cols.Size(); i++) {
	...
	cout << "WARNING: Matrix::Assemble(const Matrix &V, const ID &l): ";
	cout << " - position (" << pos_Rows << "," << pos_Cols << ") outside bounds \n";
	res = -1;
      }
    }
  }
...
Matrix::Solve(const Vector &b, Vector &x) const
{
	...
	delete [] matrixWork;
      ...
      if (matrixWork == 0) {
	cout << "WARNING: Matrix::Solve() - out of memory creating work area's\n";
	sizeDoubleWork = 0;      
	return -3;
      }
    }
...
#endif
#ifdef _DLL
	cout << "Matrix::Solve - not implemented in dll\n";
	return -1;
#endif
#else
...
Matrix::operator/=(double fact)
{
	...
	*dataPtr++ *= val;
      ...
      cout << "WARNING:Matrix::operator/= - 0 factor specified all values in Matrix set to ";
      cout << MATRIX_VERY_LARGE_VALUE << endl;
...
{
    if (fact == 0.0) {
	cout << "Matrix::operator/(const double &fact): ERROR divide-by-zero\n";
	exit(0);
    }
    Matrix result(*this);
    result /= fact;
...
Matrix::operator*(const Vector &V) const
{
    ...
    if (V.Size() != numCols) {
	cout << "Matrix::operator*(Vector): incompatable sizes\n";
	return result;
    } 
...
Matrix::operator*(const Matrix &M) const
{
	...
	cout << "Matrix::operator*(Matrix): incompatable sizes\n";
	return result;
    } 
...
Matrix::Assemble(const Matrix &V, int init_row, int init_col, double fact) 
{
  ...
  {
     cout << "WARNING: Matrix::Assemble(const Matrix &V, int init_row, int init_col, double fact): ";
     cout << "position outside bounds \n";
     res = -1;
  }
...
Matrix::AssembleTranspose(const Matrix &V, int init_row, int init_col, double fact) 
{
  ...
  {
     cout << "WARNING: Matrix::AssembleTranspose(const Matrix &V, int init_row, int init_col, double fact): ";
     cout << "position outside bounds \n";
     res = -1;
  }
...
Matrix::Extract(const Matrix &V, int init_row, int init_col, double fact) 
{
  ...
  {
     cout << "WARNING: Matrix::Extract(const Matrix &V, int init_row, int init_col, double fact): ";
     cout << "position outside bounds \n";
     res = -1;
  }
