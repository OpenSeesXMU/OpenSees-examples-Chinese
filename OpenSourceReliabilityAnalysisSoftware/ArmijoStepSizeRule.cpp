									...
									int stepNumber)
{
	...
	logfile << "Entering the Armijo rule to compute step size ..." << endl;
	logfile.flush();
	...
	if (gradNorm == 0.0) {
		cout << "ArmijoStepSizeRule::computeStepSize() - the norm " << endl
	...
	// Inform the user
	if (printFlag != 0) {
		cout << "Armijo starting gFun evaluation at distance " << u_new.Norm() << "..." << endl
			<< " .......: ";
	}
	logfile << "Armijo starting gFun evaluation at distance " << u_new.Norm() << "..." << endl;
	logfile.flush();
	...
	if (u_new.Norm()>radius) {
		...
		// Inform the user
		if (printFlag != 0) {
			cout << "Armijo skipping gFun evaluation because of hyper sphere requirement..." << endl
				<< " .......: ";
		}
		logfile << "Armijo skipping gFun evaluation because of hyper sphere requirement..." << endl;
		logfile.flush();
		...
		if (result < 0) {
			cout << "ArmijoStepSizeRule::computeStepSize() - could not set " << endl
				<< " vector u in the xu-transformation. " << endl;
			return -1;
		}
		result = theProbabilityTransformation->transform_u_to_x_andComputeJacobian();
		if (result < 0) {
			cout << "ArmijoStepSizeRule::computeStepSize() - could not  " << endl
...
/*		if (result < 0) {
			...
			FEconvergence = false;
			cout << "step size rejected due to non-converged FE analysis ..." << endl
				<< " .......: ";
		}
		*/
		...
		if (result < 0) {
			cout << "ArmijoStepSizeRule::computeStepSize() - could not  " << endl
				<< " tokenize the limit-state function. " << endl;
			return -1;
		}
	...
	while ( mustGoOn ) {
		...
		cout << "Armijo trial point rejected; reducing step size..." << endl
			<< " .......: ";
		logfile << "Armijo trial point rejected; reducing step size..." << endl;
		logfile.flush();
...
/*			if (result < 0) {
				...
				cout << "step size rejected due to non-converged FE analysis ..." << endl
					<< " .......: ";
				logfile << "step size rejected due to non-converged FE analysis ..." << endl;
			}
*/			result = theGFunEvaluator->evaluateG(x_new);
			if (result < 0) {
				cout << "ArmijoStepSizeRule::computeStepSize() - could not  " << endl
