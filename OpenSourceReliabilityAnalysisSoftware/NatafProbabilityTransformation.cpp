	...
	:ProbabilityTransformation()
{
	...
	if (theMatrixOperations == 0) {
		cout << "NatafProbabilityTransformation::NatafProbabilityTransformation() - could " << endl
	...
	if (result < 0) {
		cout << "NatafProbabilityTransformation::NatafProbabilityTransformation() - could not" << endl
...
NatafProbabilityTransformation::meanSensitivityOf_x_to_u(Vector &x, int rvNumber)
{
	...
	if (strcmp(theRV->getType(),"NORMAL")==0) {
		...
		cout << "WARNING: Cannot compute reliability sensitivity results for " << endl
			<< " type of random variable number " << rvNumber << endl;
...
NatafProbabilityTransformation::transform_u_to_x_andComputeJacobian()
{
	...
	if (result < 0) {
		cout << "NatafProbabilityTransformation::transform_u_to_x() - could not set " << endl
	...
	if (result < 0) {
		cout << "NatafProbabilityTransformation::transform_u_to_x() - could not " << endl
...
{	
	...
	for ( int i=0 ; i<nrv ; i++ )
	{
		...
		if (strcmp(theRV->getType(),"NORMAL")==0) {
			...
			if (x(i) == 0.0) {
				cout << "NatafProbabilityTransformation::getJacobian_z_x() -- Error: value " << endl
			...
			if (pdf == 0.0) {
				cout << "ERROR: NatafProbabilityTransformation::getJacobian_z_x() -- " << endl
					<< " the PDF value is zero, probably due to too large step. " << endl;
			}
...
NatafProbabilityTransformation::setCorrelationMatrix(int pertMeanOfThisRV, int pertStdvOfThisRV, double h)
{
	...
	for ( int j=1 ; j<=numberOfCorrelationCoefficients ; j++ )
	{
		...
		if (rv1Ptr->getMean() == 0.0  ||  rv2Ptr->getMean() == 0.0 ) {
			cout << "WARNING: The Nataf-transformation is currently implemented " << endl
				<< " with the assumption of non-zero mean. " << endl;
		}
		...
		if ( strcmp(typeRv1,"USERDEFINED") == 0  ||  strcmp(typeRv2,"USERDEFINED") == 0  ) {
			cout << " ... modifying correlation rho("<<rv1<<","<<rv2<<") for user-defined random variable..." << endl;
			newCorrelation = solveForCorrelation(rv1, rv2, correlation);
			cout << " ... computed correlation for Nataf standard normal variates: " << newCorrelation << endl;
		}
		// -------------- Quan Gu 2012 May ASSUMing solveForCorrelation() is correct --------------

		else if ( strcmp(typeRv1,"TRUNCATEDNORMAL") == 0  ||  strcmp(typeRv2,"TRUNCATEDNORMAL") == 0  ) {


			cout << " ... modifying correlation rho("<<rv1<<","<<rv2<<") for TRUNCATEDNORMAL random variable..." << endl;
			newCorrelation = solveForCorrelation(rv1, rv2, correlation);
			cout << " ... computed correlation for Nataf standard normal variates: " << newCorrelation << endl;
		}
		...
		/////////////////////////////////////////////////////////////////////////////////
			...
			cout << "Did not find the given combination of distributions in CorrelationModifier" << endl;
		}
...
NatafProbabilityTransformation::solveForCorrelation(int rv_i, int rv_j, double rho_original)
{
	...
	for (int i=1;  i<=100;  i++ )  {
		...
		if ( fabs(df) < 1.0e-15) {
			cout << "WARNING: NatafProbabilityTransformation::solveForCorrelation() -- " << endl
				<< " zero derivative in Newton algorithm. " << endl;
		}
		else {
			...
			if (fabs(1.0-fabs(rho_old/rho_new)) < tol) {
				...
				if (i==100) {
					cout << "WARNING: NatafProbabilityTransformation::solveForCorrelation() -- " << endl
						<< " Newton scheme did not converge. " << endl;
					result = 0.0;
					return result;
				}
				else {
