...
:SearchDirection(), MeritFunctionCheck(), HessianApproximation()
{
	...
	// Check parameters
	if (c_bar < 1.0) {
		cout << "ERROR: Parameter c_bar in SQP algorithm is invalid." << endl;
	}
	if (e_bar > 1.0) {
		cout << "ERROR: Parameter e_bar in SQP algorithm is invalid." << endl;
	}
}
							...
							Vector gradG )
{
	...
	if (stepNumber == 1) {
		if (theHessianApproximation != 0) {
			...
			cout << "WARNING: SQPsearchDirectionMeritFunctionAndHessian::computeSearchDirection() -- " << endl
				<< "theHessianApproximation has not been set! " << endl;
		}
		delta = 1.0;
		c = c_bar;
		lambda = 1.0;
...
{
	cout << "WARNING: SQPsearchDirectionMeritFunctionAndHessian::getMeritFunctionValue() --" << endl
...
{
	cout << "WARNING: SQPsearchDirectionMeritFunctionAndHessian::updateMeritParameters() --" << endl
		<< " no explicit merit function value is computed." << endl;
										  ...
										  double g_new)
{
	...
	if (alpha == 0.0) {
		cout << "ERROR: SQPsearchDirectionMeritFunctionAndHessian::check()" << endl
...
SQPsearchDirectionMeritFunctionAndHessian::setAlpha(double palpha)
{
	...
	if (alpha > 0.5) {
		cout << "ERROR: Parameter alpha in SQP algorithm is invalid." << endl;
	}
