...
: sz(size), theData(0), fromFree(0)
{
  ...
  if (size > 0) {
    ...
    if (theData == 0) {
      cout << "Vector::Vector(int) - out of memory creating vector of size " << size << endl;
...
: sz(other.sz),theData(0),fromFree(0)
{
  if (sz != 0) {
    ...
    if (theData == 0) {
      cout << "Vector::Vector(int) - out of memory creating vector of size " << sz << endl;
    }
  }
...
Vector::setData(double *newData, int size){
  ...
  if (sz <= 0) {
    cout << " Vector::Vector(double *, size) - size specified: " << size << " <= 0\n";
    sz = 0;
  }
...
Vector::resize(int newSize){
  ...
  if (newSize < 0) {
    cout << "Vector::resize) - size specified " << newSize << " <= 0\n";
    return -1;
  } 
  ...
  // otherwise if newSize is gretaer than oldSize free old space and get new space
    ...
    if (theData == 0) {
      cout << "Vector::resize() - out of memory for size " << newSize << endl;
      sz = 0;
      return -2;
    }
    sz = newSize;
...
Vector::Assemble(const Vector &V, const ID &l, double fact )
{
  ...
  for (int i=0; i<l.Size(); i++) {
    ...
    if (pos < 0)
      ...
      result = -1;
      if (pos < sz)
	cout << "Vector::Assemble() " << pos << " out of range [1, " << sz-1 << "]\n";
      else
	cout << "Vector::Assemble() " << pos << " out of range [1, "<< V.Size()-1 << "]\n";
    }
  }
  return result;
}
...
Vector::operator()(const ID &rows) const
{
  ...
  if (result.Size() != rows.Size()) {
    cout << "Vector::()(ID) - new Vector could not be constructed\n";
    return result;
  }
  ...
  for (int i=0; i<rows.Size(); i++) {
    pos = rows(i);
    if (pos <0 || pos >= sz) {
      cout << "Vector::()(ID) - invalid location " << pos << " outside range [0, " << sz-1 << "]\n";
    } else
      result(i) = (*this)(pos);
  }
  return result;
...
Vector::operator+(double fact) const
{
  Vector result(*this);
  if (result.Size() != sz) 
    cout << "Vector::operator+(double) - ran out of memory for new Vector\n";
...
{
    if (fact == 0.0) 
      cout << "Vector::operator/(double fact) - divide-by-zero error coming\n";
...
Vector::operator+(const Vector &b) const
{
  ...
  if (result.Size() != sz) {
    cout << "Vector::operator-(Vector): new Vector not of correct size \n";
    return result;
  }
  result += b;
  return result;
...
Vector::Assemble(const Vector &V, int init_pos, double fact) 
{
  ...
  {
     cout << "WARNING: Vector::Assemble(const Vector &V, int init_pos, double fact): ";
