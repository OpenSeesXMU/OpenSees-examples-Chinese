...
SearchWithStepSizeAndStepDirection::findDesignPoint(ReliabilityDomain *passedReliabilityDomain)
{
	...
	if (printFlag == 0) {
		outputFile2 << "The user has not specified to store any search points." << endl;
		outputFile2 << "This is just a dummy file. " << endl;
	}
	...
	if (startPoint == 0) {
		...
		if (result < 0) {
			cout << "SearchWithStepSizeAndStepDirection::doTheActualSearch() - " << endl
	...
	while ( i <= maxNumberOfIterations )
	{
		...
		if (printFlag != 0) {
			...
			if (printFlag == 1) {
				...
				for (iii=0; iii<x.Size(); iii++) {
					outputFile2<<setprecision(5)<<setw(15)<<x(iii)<<endl;
				}
			}
			else if (printFlag == 2) {
				outputFile2.setf(ios::scientific, ios::floatfield);
				for (iii=0; iii<u.Size(); iii++) {
					outputFile2<<setprecision(5)<<setw(15)<<u(iii)<<endl;
				}
			}
		}
		...
		if (evaluationInStepSize == 0) {
			...
			if (result < 0) {
				cout << "SearchWithStepSizeAndStepDirection::doTheActualSearch() - " << endl
					<< " could not run analysis to evaluate limit-state function. " << endl;
				return -1;
			}
			result = theGFunEvaluator->evaluateG(x);
			if (result < 0) {
				cout << "SearchWithStepSizeAndStepDirection::doTheActualSearch() - " << endl
					<< " could not tokenize limit-state function. " << endl;
				return -1;
			}
		...
		if (i == 1)	{
			...
			cout << " Limit-state function value at start point, g=" << gFunctionValue << endl;
			cout << " STEP #0: ";
		...
		if (result > 0)  {
			...
			if (printFlag != 0) {
				if (printFlag == 3) {
					...
					if (result < 0) {
						cout << "SearchWithStepSizeAndStepDirection::doTheActualSearch() - " << endl
							<< " could not transform from u to x." << endl;
						return -1;
					}
					x = theProbabilityTransformation->get_x();
	...
	cout << "Maximum number of iterations was reached before convergence." << endl;
