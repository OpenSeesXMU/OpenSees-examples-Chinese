...
BJmatrix::BJmatrix(char *flag, int dimension ): // create an ident BJmatrix
  nDarray((char*)"NO")           // with base class constructor cancelation
  {
...
#define BSIZE 120
  ...
  if ((from = fopen(initfile,"r")) == 0)
    error((char*) "cannot open BJmatrix initializer file", initfile);
    ...
    for ( int col=1 ; col <= columns ; col++ )
      {
        ...
        if(ferror(from))
          error((char*)"problem with BJmatrix initializer file ", initfile);
      }
  fclose(from);
}
...
//Kai    error("cannot open BJmatrix output file", outfile);
  write_standard(outfile, (char*) "this is a test BJmatrix output");
}
...
void BJmatrix::write_standard(char *filename, char *msg)
  {
    ...
    if ((to = fopen(filename,"w")) == NULL)
      error((char*)"cannot open or create BJmatrix output file",filename);
        ...
        for( int col=0; col<cols() ; col++ )
          {
            ...
            if(ferror(to))
              error((char*)"problem with BJmatrix output file ", filename);
          }
        fprintf(to, "\n");
      }
}
...
BJmatrix BJmatrix::operator*( BJmatrix & arg)
  {
    if( cols() != arg.rows())
      error((char*)"# rows of second mat must equal "
               "# cols of first for multiply#");
...
double BJmatrix::mmin()
  {
    double temp;
    if ( rows()<=0 || cols()<=0)
      error((char*)"bad BJmatrix size for min ()");
...
double BJmatrix::mmax()
  {
    double temp;
    if( rows()<=0 || cols()<=0)
      error((char*)"bad BJmatrix size for max()");
...
double BJmatrix::determinant()
  {
    if( rows() != cols() )
      error((char*)"BJmatrix must be square for determinant()");
...
BJmatrix BJmatrix::inverse()
  {
    if( rows()!=cols() )
      error((char*)"BJmatrix must be square for inverse()");
    BJmatrix Y((char*)"I", rows()); // create an identity BJmatrix
    BJmatrix indx(cols());   // create the "index vector"
...
void BJmatrix::copy_column( BJmatrix & mm, int from_col, int to_col)
  {
    if( rows()!=mm.rows() )
      error((char*)"number of rows must be equal for copy_column()");
    for( int row=0 ; row<rows() ; row++ )
...
void BJmatrix::deepcopy(BJmatrix & from, BJmatrix & to)
  {
    if( from.rows()!=to.rows() || from.cols()!=to.cols() )
      error((char*)"matrices must be equal dimensions for deepcopy()");
...
//    ::fprintf(stderr,"BJmatrix BJmatrix::scale( )\n"); 
    ...
    double temp;
    if( rows()<=0 || cols()<=0 )
      error((char*)"bad BJmatrix size for scale()");
    if( rows()!=cols() )
      error((char*)"BJmatrix must be square for scale()");
    ...
    for ( int col=0 ; col<cols() ; col++ )
      {
        ...
        for( int row=0 ; row<rows() ; row++ )
          ...
          if( fabs(maximum) <= double_eps )
            error((char*)"singular BJmatrix in scale()");
...
//*/
    if( rows()!=cols() )
      error((char*)"Matrix must be square to L-U decompose#\n");
    d = 1; // parity check
...
//*/
    if( rows() != cols() )
      error ((char*)"non-square lu_decomp BJmatrix in lu_back_subst()");
    if( rows() != b.rows() )
      error((char*)"wrong size B vector passed to lu_back_subst()");
    if( rows() != indx.rows() )
      error((char*)"wrong size indx vector passed to lu_back_subst()");
    int row, col, ll;
    int ii = 0;
    double sum;
    for( col=0 ; col < cols() ; col++ )
