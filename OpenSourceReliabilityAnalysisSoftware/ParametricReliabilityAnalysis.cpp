...
ParametricReliabilityAnalysis::analyze(void)
{
	...
	cout << "Fragility Analysis is running ... " << endl;
	...
	for (int lsf=1; lsf<=numLsf; lsf++ ) {
		...
		if (theLimitStateFunction == 0) {
			cout << "ParametricReliabilityAnalysis::analyze() - could not find" << endl
		...
		outputFile << "#                                                                     #" << endl;
		outputFile << "#                                                                     #" << endl;
		outputFile << "#                    Failure probability     Estimated probability    #" << endl;
		outputFile << "#    Parameter       estimate (fragility)     densitity function      #" << endl;
		outputFile << "#     value               (CDF)                    (PDF)              #" << endl;
		...
		for (int counter=0; counter<(numIntervals+1); counter++) {
			...
			if (numPos != 0) {
				...
				if (theParameterPositioner == 0) {
					cout << "ParametricReliabilityAnalysis::analyze() -- The parameter number in the " << endl
						<< " fragility analysis object does not match the parameter number " << endl
						<< " being set by the parameter positioner." << endl;
				}
			}
			else {
				char separators[5] = "_}{";
				...
				while ( tokenPtr != NULL ) {
					if ( strcmp(tokenPtr, "par") == 0) {
						...
						if (par==parameterNumber) {
							...
							cout << "ParametricReliabilityAnalysis::analyze() -- The parameter number " << endl
								<< " in the limit-state function does not match the parameter " << endl
								<< " number in the fragility analysis object." << endl;
						}
					}
					tokenPtr = strtok( NULL, separators);
				}
			...
			if (pf(counter)==-1.0) {
				...
				outputFile <<setprecision(3)<<setw(11)<<pdf(counter)<<"           #" << endl;
			}
			outputFile.flush();
	...
	cout << "Fragility Analysis completed." << endl;
