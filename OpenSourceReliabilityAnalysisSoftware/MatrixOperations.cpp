...
MatrixOperations::getLowerCholesky()
{
	if (theLowerCholesky == 0) {
		cout << "MatrixOperations::getLowerCholesky() - this" << endl
...
MatrixOperations::getInverseLowerCholesky()
{
	if (theInverseLowerCholesky == 0) {
		cout << "MatrixOperations::getInverseLowerCholesky() - this" << endl
			<< " matrix has not been computed." << endl;
		return (*theMatrix);
	}
...
{
	if (theInverse == 0) {
		cout << "MatrixOperations::getInverse() - this" << endl
			<< " matrix has not been computed." << endl;
		return (*theMatrix);
	}
...
{
	if (theTranspose == 0) {
		cout << "MatrixOperations::getTranspose() - this" << endl
			<< " matrix has not been computed." << endl;
		return (*theMatrix);
	}
...
{
	if (theSquareRoot == 0) {
		cout << "MatrixOperations::getSquareRoot() - this" << endl
			<< " matrix has not been computed." << endl;
		return (*theMatrix);
	}
		...
		for ( j=0 ; j<sizeOfPassedMatrix ; j++ )
		{
			...
			if ( i == j )
			{
				if ( ( passedMatrix(i,j) - sumOfLambda_i_k_squared ) < 1.0e-8) {
					cout << "WARNING: MatrixOperations::computeLowerCholesky()" << endl
						<< " ... matrix may be close to singular. " << endl;
				}
		...
		for ( j=0 ; j<sizeOfPassedMatrix ; j++ )
		{
			...
			if ( i > j )
			{
				if ( fabs(lambda(i,i)) < 1.0e-8) {
					cout << "WARNING: MatrixOperations::computeInverseLowerCholesky()" << endl
						<< " ... matrix may be close to singular. " << endl;
				}
...
MatrixOperations::computeCholeskyAndItsInverse()
{
	...
	for ( i=0 ; i<sizeOfPassedMatrix ; i++ ) 
	{
		for ( j=0 ; j<sizeOfPassedMatrix ; j++ )
		{
			...
			{
				if ( ( passedMatrix(i,j) - sumOfLambda_i_k_squared ) < 1.0e-8) {
					cout << "WARNING: MatrixOperations::computeCholeskyAndItsInverse()" << endl
						<< " ... matrix may be close to singular. " << endl;
				}
				lambda(i,j) = sqrt ( passedMatrix(i,j) - sumOfLambda_i_k_squared );
			}
...
MatrixOperations::computeTrace()
{
	...
	if (numberOfColumns != numberOfRows) {
		cout << "MatrixOperations::computeTrace() - can not" << endl
...
{
	int beginMod = pBeginMod;
	int endMod = pEndMod;

	Matrix * A = theMatrix;

	//  cout<< "hessian:"<< * theMatrix<<endl;

	if (A == 0) { cout << "error, Hessian does not exist in MatrixOperator::performEigenAnalysis !" << endl; exit(-1); }

	// Number of equations
	int n = A->noRows();
	if (n != A->noCols()) {
		cout << "MatrixOperations::performEigenAnalysis wrong. m!=n" << endl;
		exit(-1);
	}
...
/*	// Call the LAPACK eigenvalue subroutine
	...
	*/
	...
	if (info < 0) {
		cout << "Hessian::performEigenAnalysis() -- invalid argument number " << -info << " passed to LAPACK dsbevx\n";
		return info;
	}

	if (info > 0) {
		cout << "Hessian::performEigenAnalysis() -- LAPACK dsbevx returned error code " << info << endl;
		return -info;
	}

	if (m < numModes) {
		cout << "Hessian::performEigenAnalysis() -- LAPACK dsbevx only computed " << m << " eigenvalues, " <<
...
/*
int 
MatrixOperations::performEigenAnalysis(int pBeginMod, int pEndMod)
{
...
MatrixOperations::getEigenvector(int mode)
{
  if (mode < beginMod || mode > endMod) {
    cout << "Hessian::getEigenVector() -- mode " << mode << " is out of range (1 - "
	   << numModes << ")\n";
  ...
  if (eigenvector != 0) {
    ...
    cout << "Hessian::getEigenVector() -- eigenvectors not yet computed\n";
    eigenV->Zero();
  }      
...
{
  if (mode < beginMod || mode > endMod) {
    cout << "Hessian::getEigenvalue() -- mode " << mode << " is out of range (1 - "
	   << numModes << ")\n";
  ...
  if (eigenvalue != 0)
    ...
    cout << "Hessian::getEigenvalue() -- eigenvalues not yet computed\n";
    return 0.0;
  }      
}
...
MatrixOperations::setSizeOfEigenVector(int newSize)
{
  ...
  if (eigenV == 0 || eigenV->Size() != newSize) {
    ...
    if (eigenV == 0 || eigenV->Size() != newSize) {
      cout << "Hessian::ssetSize() -- ran out of memory for eigenvector of size " << newSize << endl;
      return -2;	    
    }
  }
