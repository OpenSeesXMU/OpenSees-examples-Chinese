...
FiniteDifferenceGradGEvaluator::computeGradG(double gFunValue, Vector passed_x)
{
	...
	if (reComputeG) {
		...
		if (result < 0) {
			cout << "FiniteDifferenceGradGEvaluator::evaluate_grad_g() - " << endl
	...
	if (doGradientCheck) {
		...
		for (int ffd=0; ffd<grad_g->Size(); ffd++) {
			cout << "FFD("<< (ffd+1) << ") = " << (*grad_g)(ffd) << endl;
			sprintf(myString,"%20.16e ",(*grad_g)(ffd));
			outputFile << myString << endl;
		}
		outputFile.close();
		cout << "PRESS Ctrl+C TO TERMINATE APPLICATION!" << endl;
		while(true) {
		}
	}
